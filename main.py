"""
Jogo da Cobrinha com IA (Dijkstra) — várias partidas simultâneas em UMA
única janela (grid de mini-tabuleiros), otimizado para rodar muitos
passos de lógica por segundo.

Controles:
    Setas       -> mover a cobra (só tem efeito com IA desligada, veja Config.ai_enabled)
    Espaço      -> alterna entre a velocidade "turbo" e a velocidade alternativa
    Enter       -> reinicia os jogos que já perderam

Sobre performance (o que fazer se ainda estiver lento):
  - `steps_per_frame` é a principal alavanca de velocidade: quantos passos
    de lógica cada tabuleiro executa a cada quadro desenhado. Aumente esse
    valor para ir mais rápido sem precisar desenhar/atualizar a tela mais vezes.
  - `animate_movement=False` (padrão) desenha só a célula final de cada passo,
    e só redesenha as células que mudaram (muito mais barato). Deixe True
    só se quiser ver a cobra "deslizando" suavemente (recomendado com
    steps_per_frame=1, para demonstração).
  - `show_ai_path=True` obriga a redesenhar todo o caminho calculado a cada
    passo, o que custa mais caro — deixe False para velocidade máxima.
"""
